...
def voltage_timeseries():

  user_voltage_data = {}

  for user in user_to_sensor_id.keys():

    user_voltage_data[user] = []
    user_voltage_mean = []
    user_voltage_std = []
    user_voltage_time = []
    ...
    for record in timeseries_cursor:
      ...
      mean = record['mean']
      std = record['std']
      user_voltage_mean.append(mean)
      user_voltage_std.append(std)
      user_voltage_time.append(record['utctime'])

    user_voltage_data[user] = [user_voltage_mean, user_voltage_std, user_voltage_time]

  return user_voltage_data

# call voltage_timeseries
# user_voltage_data = voltage_timeseries()
# print user_voltage_data


with open('user_voltage_data.json') as f:    
    user_voltage_data = json.load(f)
# print dateutil.parser.parse(user_voltage_data['KIT1-0003'][2])
# print user_voltage_data['KIT1-0003'][0]
...
# writing exported data from mongo database into a json file
# with open('user_voltage_data.json', 'w') as f :
#     json.dump(user_voltage_data, f, default=json_util.default)
...
def plot_combined():
  ...
  # ax1.plot_date(connec_data.connec['KIT1-0003'][1], connec_data.connec['KIT1-0003'][0], fmt='b-') # x = array of dates, y = array of numbers        
