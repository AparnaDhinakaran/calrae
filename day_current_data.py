...
def current_timeseries():
	...
	for user in user_to_sensor_id.keys():
		...
		current_timeseries_curser = db.day.find( { "utctime":{ "$gte": d1,
	      "$lte": d2}, "sensor": internal_id,
	      "tag": "i_rms" })
	
		for record in current_timeseries_curser:
			time = record['utctime']
			mean = record['mean']
			std = record['std']
			user_current_time.append(time.isoformat())
			user_current_mean.append(mean)
			user_current_std.append(std)

		user_current_data[user] = [user_current_mean, user_current_std, user_current_time]

	return user_current_data

user_current_data = current_timeseries()

print user_current_data

with open('./user_current_data.json', 'w') as f :
     json.dump(user_current_data, f, default=json_util.default)
